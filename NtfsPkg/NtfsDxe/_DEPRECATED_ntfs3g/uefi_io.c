...
/**
 * ntfs_device_uefi_io_open - Open a device and lock it exclusively
 * @dev:
 * @flags:
 *
 * Description...
...
/**
 * ntfs_device_uefi_io_close - Close the device, releasing the lock
 * @dev:
 *
 * Description...
 *
...
static s64 ntfs_device_uefi_io_seek(struct ntfs_device *dev, s64 offset,
		int whence)
{
	return 0; //lseek(DEV_FD(dev), offset, whence);
}
...
static s64 ntfs_device_uefi_io_read(struct ntfs_device *dev, void *buf,
		s64 count)
{
	return 0; //read(DEV_FD(dev), buf, count);
}
		...
		s64 count, s64 offset)
{
	...
	NDevSetDirty(dev);*/
	return 0; //pwrite(DEV_FD(dev), buf, count, offset);
}
 ...
 * Returns:
 */
static int ntfs_device_uefi_io_ioctl(struct ntfs_device *dev, int request,
		void *argp)
{

	return 0; //ioctl(DEV_FD(dev), request, argp);
}
